...
CHUNK_SIZE = 800        # Re-configured to 800 characters per chunk
...
def embed_chunks(chunks: list[dict]) -> list[dict]:
    ...
    if provider == "openai":
        ...
        res = genai.embed_content(model=model_name, contents=texts)
        embeddings = res['embedding']
    else:
        from sentence_transformers import SentenceTransformer
        model = SentenceTransformer(EMBEDDING_MODEL)
        embeddings_arr = model.encode(texts, show_progress_bar=True)
...
def index_to_vectorstore(chunks: list[dict]):
    ...
    """
    if not chunks:
        print("[WARN] Khong co chunk nao de index.")
        return
